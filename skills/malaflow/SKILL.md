...
5. After explicit confirmation, call `create_order_and_wait` with database menu item IDs and quantities.
6. `create_order_and_wait` creates the order, then waits up to 5 minutes, polling every 10 seconds, until the restaurant accepts with a pickup number, rejects, or the order is cancelled.
7. If `create_order_and_wait` returns `accepted`, tell the user the pickup number.
8. If it returns `rejected` or `cancelled`, tell the user plainly.
9. If `create_order_and_wait` is unavailable, fallback to `create_order`, keep the returned `order_id`, then immediately call `wait_for_order_result`.
10. If `wait_for_order_result` is also unavailable, fallback to calling `get_order_status` every 10 seconds for up to 5 minutes.
11. If the user asks to cancel before acceptance, call `cancel_order` only for a submitted order.
